...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	accountModelCapabilityRepository := repository.NewAccountModelCapabilityRepository(db)
	accountModelCapabilityService := service.NewAccountModelCapabilityService(accountModelCapabilityRepository)
	...
	playgroundService := service.NewPlaygroundService(userRepository, groupRepository, apiKeyService, subscriptionService, accountModelCapabilityService, gatewayService)
	...
	playgroundHandler := handler.NewPlaygroundHandler(playgroundService, openAIGatewayHandler, billingCacheService)
	...
	accountUpstreamModelService := service.NewAccountUpstreamModelService(accountRepository, accountModelCapabilityService, modelCatalogService)
	accountUpstreamModelHandler := admin.NewAccountUpstreamModelHandler(accountUpstreamModelService)
	...
	modelCatalogHandler := admin.NewModelCatalogHandler(modelCatalogService, accountModelCapabilityService)
	modelVendorService := service.NewModelVendorService(modelCatalogRepository)
	modelVendorHandler := admin.NewModelVendorHandler(modelVendorService)
	...
	aiRequestLogRepository := repository.NewAIRequestLogRepository(db)
	aiRequestLogService := service.NewAIRequestLogService(aiRequestLogRepository, settingRepository)
	aiRequestLogHandler := admin.NewAIRequestLogHandler(aiRequestLogService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, accountUpstreamModelHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, modelCatalogHandler, modelVendorHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, paymentHandler, aiRequestLogHandler)
	...
	images2Service := service.NewImages2Service(settingService, apiKeyService, userService, groupService)
	images2Handler := handler.NewImages2Handler(images2Service, openAIGatewayHandler)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, playgroundHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, images2Handler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddleware(apiKeyService, subscriptionService, configConfig)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, apiKeyService, subscriptionService, opsService, aiRequestLogService, settingService, redisClient)
