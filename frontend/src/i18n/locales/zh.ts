export default {
  ...
  images2: {
    ...
    currentResultBadge: '当前编辑图',
    currentResultAlt: '当前结果图',
    editPreserveAspectTitle: '编辑当前图片时将保留原图比例',
    editPreserveAspectBody: '当前结果图尺寸约为 {width} × {height}，比例约 {ratio}:1。点“编辑当前图片”时会优先沿用这个比例。',
    editPreserveAspectBodyUnknown: '点“编辑当前图片”时会优先保留当前结果图的原始比例，不再强制套用下面的固定尺寸选项。',
    editHint: '',
    resultEditHint: '想继续细化这张图的话，直接调整提示词后点击“基于当前图编辑”就行，系统会把当前图片和新提示词一起提交。',
    resultActionHintRegenerateLabel: '重新生成：',
    resultActionHintRegenerateBody: '忽略当前结果图，按提示词重新出图，并参考你上传的参考图。',
    resultActionHintEditLabel: '编辑当前图片：',
    resultActionHintEditBody: '在当前结果图上继续修改，也会参考你上传的参考图。',
    generating: '生成中...',
    newImage: '新图片',
